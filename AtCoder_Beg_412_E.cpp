...
void solve(){
  ...
  int ans=1;
  vector<int>prime;
  findprime(1e7+1,prime);
  for(int i=0;i<prime.size()&&prime[i]<=r;i++)
  {
    //if(prime[i]>l)ans++;
    int val=prime[i];
    while(val<=(r+prime[i]-1)/prime[i])
    {
        val*=prime[i];
        if(val>l)ans++;
    }
  ...
  {
    int x=prime[i];
    for(int j=max(l/prime[i],prime[i]);j*prime[i]<=r;j++)
    {
        if(j*prime[i]>l)m[j*prime[i]]=1;
    }
  }
  
  cout<<ans+newVal-m.size()<<endl;
}
